/**
 * QuantEdge Cloudflare Worker — kite.js v4.3
 *
    ...
    if ((path === "/" || path === "") && method === "GET") {
      ...
      if (!symbol) {
        ...
        return cors({ kite: token ? "connected" : "disconnected",
                      status: "success", version: "4.3" });
      }

      if (type === "fundamentals") {
        // BUG FIX (v4.3): Previous handler returned Kite quote data (price/volume).
        // Browser parser (_fetchFundamentalsYahoo) expected Yahoo quoteSummary shape
        // with financialData/defaultKeyStatistics/summaryDetail — causing silent null
        // and all fundamental cards showing N/A.
        // Fix: proxy Yahoo Finance quoteSummary directly, return quoteSummary result
        // so existing browser parsing logic works without any browser-side changes.
        try {
          const cleanSym = symbol.replace(/\.NS$/, "").toUpperCase();
          const yfSym    = encodeURIComponent(cleanSym + ".NS");
          const modules  = "financialData,defaultKeyStatistics,summaryDetail";
          const headers  = { "User-Agent": "Mozilla/5.0", "Accept": "application/json" };
          const urls = [
            "https://query1.finance.yahoo.com/v10/finance/quoteSummary/" + yfSym + "?modules=" + modules,
          ...
          for (let ui = 0; ui < urls.length; ui++) {
            try {
              ...
              let res;
              try {
                res = await fetch(urls[ui], { signal: ctrl.signal, headers });
              } finally {
                clearTimeout(timer);
              }
              if (!res.ok) continue;
              const d = await res.json();
              if (d && d.quoteSummary && d.quoteSummary.result && d.quoteSummary.result[0]) {
                yfData = d;
                break;
              }
            } catch (_) { continue; }
          ...
          if (!yfData) throw new Error("Yahoo Finance unavailable for " + cleanSym);

          // Return full quoteSummary so browser _fetchFundamentalsYahoo() parses
          // it with its existing financialData/defaultKeyStatistics/summaryDetail logic.
          return cors({
            status: "success",
            source: "yahoo",
            symbol: cleanSym,
          ...
          return cors({ status: "error", source: "yahoo",
                        message: e.message, quoteSummary: null }, 200);
        }
      }
