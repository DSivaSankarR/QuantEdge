...
// Used for: global indices (^VIX, ^GSPC, ^NSEI), futures (CL=F), forex (INR=X)
// Also used as fallback when Kite historical fails for NSE stocks
async function proxyYahooFinance(symbol, interval, range) {
  ...
  for (const yfUrl of urls) {
    try {
      ...
      // Return as-is — index.html already knows how to parse Yahoo Finance format
...
export default {
  async fetch(request, env) {
    ...
    if ((path === "/" || path === "") && method === "GET") {
      ...
      // Routes: NSE stocks → Kite historical API (live, accurate)
      //         Global symbols (^VIX, CL=F, ^GSPC, INR=X, ^NSEI etc) → Yahoo Finance proxy
      const cleanSym = symbol.replace(/\.NS$|\.BO$/, "").toUpperCase();

      // Detect non-NSE symbols: indices (^), futures (=F), forex (=X)
      const isGlobalSymbol = symbol.startsWith("^") || symbol.includes("=F") ||
                             symbol.includes("=X") || symbol.startsWith("%5E");

      if (isGlobalSymbol) {
        // ── Yahoo Finance proxy for global symbols ──
        return await proxyYahooFinance(symbol, interval, range);
      }

      // ── Kite historical for NSE stocks ──
      try {
        const token = await getToken(env);
        ...
        // Get instrument token via quote
        ...
        // Return in Yahoo Finance format — index.html parser expects this shape
        return cors({
          status: "success",
          source: "kite",
          chart: {
        ...
        // Kite failed for this NSE stock — fall back to Yahoo Finance
        return await proxyYahooFinance(symbol, interval, range);
      }
    }
    ...
    // ── /callback + /auth (alias — registered redirect URL in Zerodha Kite app) ──
