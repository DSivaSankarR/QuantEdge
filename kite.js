/**
 * QuantEdge Cloudflare Worker — kite.js v3.0
 *
 * Changelog v3.0:
 *   All v2.5 routes preserved unchanged.
 *   New additions:
 *
 *   PRIORITY 1 — Telegram Inline Buttons + Callback Handler
 *     POST /telegram/callback  — handles BUY/WATCH/REJECT from Telegram inline buttons
 *     Validates HMAC signature + signal expiry before any action
 *
 *   PRIORITY 2 — Daily Auth Reminder (scheduled cron)
 *     Cron: 03:15 UTC (08:45 IST) Mon–Fri
 *     Sends Telegram message with Kite login link
 *
 *   PRIORITY 3 — Scheduled Discovery Scan Trigger (scheduled cron)
 *     Cron: 03:45 UTC (09:15 IST) Mon–Fri
 *     Checks for valid token → sends Telegram deep-link to trigger scan
 *     (Full server-side scan is Phase 2 — bridge approach used here)
 *
 *   PRIORITY 4 — Position Monitor (scheduled cron)
 *     Cron: every 30 min during market hours (03:45–10:00 UTC = 09:15–15:30 IST)
 *     Checks active GTTs → alerts on trigger/SL/target
 *
 *   PRIORITY 5 — Daily Summary (scheduled cron)
 *     Cron: 10:30 UTC (16:00 IST) Mon–Fri
 *     Sends day's activity summary to Telegram
 *
 *   NEW KV KEYS:
 *     qe_db_universe      — cached universe list (JSON array)
 *     qe_db_universe_ts   — universe cache timestamp
 *     qe_signals          — active signals map (signalId → payload)
 *     qe_gtt_log          — GTT placement audit log (JSON array)
 *     qe_position_log     — open positions for monitoring
...
export default {
  ...
  async scheduled(event, env, ctx) {
    ...
    // Every 30 min 03:45–10:00 UTC Mon–Fri = 09:15–15:30 IST — Position monitor
    if (cron === "*/30 3-10 * * 1-5") {
